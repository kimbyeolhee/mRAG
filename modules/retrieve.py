...
class Retrieve:
    ...
    def encode_and_save(self, dataset, save_path, query_or_doc, chunk_size=150000):
        ...
        for i, batch in tqdm(enumerate(dataloader), total=total_n_batches, desc=f'Encoding: {self.model.model_name}', file=sys.stderr):
            ...
            if i % save_every_n_batches == 0 or i == total_n_batches - 1:
    ...
    def retrieve(self, dataset, query_embeds_path, doc_embeds_path, top_k_documents, return_docs=False, overwrite_index=False):
        ...
        if self.model.model_name == "bm25":
            ...
            # seperate query embedding in chunks
            chunks = torch.split(query_embeds, self.batch_size_sim, dim=0)
            scores_sorted_topk, indices_sorted_topk, embeds_sorted_topk = list(), list(), list()

            emb_files = glob.glob(f"{doc_embeds_path}/*.pt")
            sorted_emb_files = sorted(emb_files, key=lambda x: int("".join(filter(str.isdigit, x))))

            doc_embeds = list()
            for emb_file in tqdm(sorted_emb_files, total=len(sorted_emb_files), desc="Loading doc embeddings"):
                emb_chunk = torch.load(emb_file)
                doc_embeds.append(emb_chunk)

            for chunk in tqdm(chunks, desc=f"Retrieving top {top_k_documents} documents", total=len(chunks)):
                scores_sorted_topk_chunk, indices_sorted_topk_chunk, embeds_sorted_topk_chunk = self.load_collection_and_retrieve(chunk, doc_embeds, top_k_documents, dataset_size=len(dataset['doc']),return_embeddings=False)
    ...
    def load_collection_and_retrieve(self, emb_q, doc_embeds, top_k_documents, detach_and_cpu=True, return_embeddings=False, dataset_size=None):
        """
            for memory efficiency, chunk the query embedding and doc embeddings
            and retrieve top k documents for each chunk
